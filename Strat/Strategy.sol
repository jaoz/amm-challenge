...
contract Strategy is AMMStrategyBase {
    ...
    function _updateSideState(
        ...
        if (pred > obsShare) {
            ...
            if (feeBps < NORM_BPS && kappa > KAPPA_MIN + err / 8) {
                kappa -= err / 8;
            }
        } else {
            ...
            if (feeBps <= NORM_BPS) {
                kappa = clamp(kappa + err / 8, KAPPA_MIN, KAPPA_MAX);
            }
        }
        ...
        kappa = _ewma(kappa, KAPPA_INIT, A_KAPPA / 4);
    ...
    {
        if (feeBps <= NORM_BPS) {
            return clamp(baseShare + kappa * (NORM_BPS - feeBps), 1e16, 99e16);
        }
        ...
        returns (uint256)
    {
        ...
        return clamp(obsShare + kappa * (feeBps - NORM_BPS), 1e16, 99e16);
    }
