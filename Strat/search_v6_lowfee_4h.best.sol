...
contract Strategy is AMMStrategyBase {
    uint256 private constant A_ARB = 12e16;       // 0.10
    uint256 private constant A_VOL = 22e16;       // 0.22
    uint256 private constant A_RETAIL = 10e16;     // 0.07
    uint256 private constant A_PRICE_ARB = 20e16; // 0.18
    uint256 private constant A_PRICE_SPOT = 6e16; // 0.04
    ...
    function getName() external pure override returns (string memory) {
        return "WorldState_BandShift_search_best";
    }
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (bidPrev == 0) bidPrev = bpsToWad(36);
        if (askPrev == 0) askPrev = bpsToWad(36);
        ...
        bidBps = _clampBps(bidBps, 30, 44);
        askBps = _clampBps(askBps, 30, 44);
    ...
    function _baseFromState(
        ...
        uint256 base = tradeCount < 180 ? 47 : 47;

        // sigma around 8.8-10.1 bps.
        if (volHat > bpsToWad(105) / 10) {
            if (base > 7) base -= 7;
