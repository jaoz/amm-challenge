...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant A_LAMBDA_FAST = 27e16;
    uint256 private constant A_LAMBDA_SLOW = 10e16;
    uint256 private constant A_GAP_FAST = 28e16;
    uint256 private constant A_GAP_SLOW = 10e16;
    uint256 private constant A_FLOW_FAST = 25e16;
    uint256 private constant A_FLOW_SLOW = 9e16;

    uint256 private constant DISCOVERY_TRADES = 220;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool discovery = tradeCount < DISCOVERY_TRADES;
        uint256 aLambda = discovery ? A_LAMBDA_FAST : A_LAMBDA_SLOW;
        uint256 aGap = discovery ? A_GAP_FAST : A_GAP_SLOW;
        uint256 aFlow = discovery ? A_FLOW_FAST : A_FLOW_SLOW;

        if (trade.timestamp != lastTs) {
            if (lastTs != 0 || stepTrades > 0) {
                uint256 dt = trade.timestamp > lastTs ? (trade.timestamp - lastTs) : 1;
                gapHat = _ewma(gapHat, _gapObs(dt), aGap);
                flowHat = _ewma(flowHat, _flowObs(stepTrades), aFlow);
                lambdaHat = _ewma(lambdaHat, _stepLambdaObs(stepRetail, stepArb, stepTrades, dt), aLambda);
            }
        ...
        if (probableArb) {
            ...
            lastArbP = pObs;
            lambdaHat = _ewma(lambdaHat, 0, aLambda);
        } else {
            stepRetail += 1;
            ...
            lambdaHat = _ewma(lambdaHat, _tradeLambdaObs(stepTrades), aLambda);
        }

        uint256 lambdaProxy = (lambdaHat * 6 + gapHat * 2 + flowHat * 2) / 10;
        tradeCount += 1;

        uint256 baseBps = _baseFromState(lambdaProxy, arbProb, volHat, tradeCount);
        ...
        uint256 quoteMin = tradeCount < DISCOVERY_TRADES ? 12 : 14;
        uint256 quoteMax = tradeCount < DISCOVERY_TRADES ? 86 : 78;
        bidBps = _clampBps(bidBps, quoteMin, quoteMax);
        askBps = _clampBps(askBps, quoteMin, quoteMax);
    ...
    function _baseFromState(uint256 lambdaProxy, uint256 arbProb, uint256 volHat, uint256 tradeCount)
        internal
        pure
        returns (uint256)
    {
        uint256 base = tradeCount < DISCOVERY_TRADES ? 61 : 50;
        if (tradeCount < (DISCOVERY_TRADES / 2)) base += 5;
    ...
    function _stepLambdaObs(uint256 stepRetail, uint256 stepArb, uint256 stepTrades, uint256 dt)
        ...
        returns (uint256)
    {
        if (stepTrades == 0) return dt > 4 ? 18e16 : 34e16;
        if (stepArb == 0) {
