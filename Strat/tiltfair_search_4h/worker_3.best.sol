...
contract Strategy is AMMStrategyBase {
    uint256 private constant A_ARB = 8e16;
    uint256 private constant A_VOL = 27e16;
    uint256 private constant A_RETAIL = 6e16;
    uint256 private constant A_PRICE_ARB = 18e16;
    uint256 private constant A_PRICE_SPOT = 7e16;

    uint256 private constant FAIR_T1 = 11;
    uint256 private constant FAIR_T2 = 22;
    uint256 private constant FAIR_T3 = 34;
    uint256 private constant FAIR_S1 = 4;
    uint256 private constant FAIR_S2 = 6;
    uint256 private constant FAIR_S3 = 12;

    uint256 private constant INV_T1 = 184;
    uint256 private constant INV_T2 = 480;
    uint256 private constant INV_T3 = 588;
    uint256 private constant INV_S1 = 5;
    uint256 private constant INV_S2 = 9;
    uint256 private constant INV_S3 = 12;
    uint256 private constant INV_CONFLICT_DIV = 4;
    ...
    function getName() external pure override returns (string memory) {
        return "WorldState_TiltFair_best_worker_3";
    }
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (bidPrev == 0) bidPrev = bpsToWad(69);
        if (askPrev == 0) askPrev = bpsToWad(69);
        ...
        if (stepTrades == 1) {
            ...
            if (directionOk && relToHat <= bpsToWad(144)) probableArb = true;
        }
        ...
        bidBps = _clampBps(bidBps, 36, 100);
        askBps = _clampBps(askBps, 36, 100);
    ...
    {
        uint256 base = tradeCount < 180 ? 23 : 19;

        if (volHat > bpsToWad(108) / 10) {
            if (base > 9) base -= 9;
        } else if (volHat > bpsToWad(95) / 10) {
            if (base > 4) base -= 4;
        ...
        if (arbProb > 65e16) {
            if (base > 4) base -= 4;
        } else if (arbProb < 46e16) {
            base += 7;
        }

        if (retailHat > 67e16) {
            base += 3;
        } else if (retailHat < 52e16) {
            if (base > 1) base -= 1;
        }

        return _clampBps(base, 16, 86);
    }
