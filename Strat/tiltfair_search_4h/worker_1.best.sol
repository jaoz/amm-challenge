...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant A_RETAIL = 8e16;
    uint256 private constant A_PRICE_ARB = 17e16;
    uint256 private constant A_PRICE_SPOT = 2e16;

    uint256 private constant FAIR_T1 = 9;
    uint256 private constant FAIR_T2 = 10;
    uint256 private constant FAIR_T3 = 23;
    uint256 private constant FAIR_S1 = 3;
    uint256 private constant FAIR_S2 = 3;
    uint256 private constant FAIR_S3 = 5;

    uint256 private constant INV_T1 = 290;
    uint256 private constant INV_T2 = 437;
    uint256 private constant INV_T3 = 694;
    ...
    function getName() external pure override returns (string memory) {
        return "WorldState_TiltFair_best_worker_1";
    }
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (bidPrev == 0) bidPrev = bpsToWad(30);
        if (askPrev == 0) askPrev = bpsToWad(30);
        ...
        if (stepTrades == 1) {
            ...
            if (directionOk && relToHat <= bpsToWad(170)) probableArb = true;
        }
        ...
        bidBps = _clampBps(bidBps, 36, 104);
        askBps = _clampBps(askBps, 36, 104);
    ...
    {
        uint256 base = tradeCount < 180 ? 29 : 25;

        if (volHat > bpsToWad(100) / 10) {
            if (base > 13) base -= 13;
        ...
        if (retailHat > 75e16) {
            base += 5;
        } else if (retailHat < 32e16) {
            if (base > 1) base -= 1;
        }

        return _clampBps(base, 22, 109);
    }
