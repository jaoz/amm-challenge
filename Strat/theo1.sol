...
contract Strategy is AMMStrategyBase {
    ...
    // --- Cubic Toxicity ---
    uint256 constant TOX_CUBIC_COEF = 1500000000000000000;

    // --- Trade-Tox Boost ---
    uint256 constant TRADE_TOX_BOOST = 250000000000000000;

    // --- Asymmetric Stale Dir ---
    uint256 constant STALE_ATTRACT_FRAC = 1124000000000000000; // 1.124

    // --- state caps ---
    uint256 constant RET_CAP = 100000000000000000; // 10%
    ...
    uint256 constant ARB_TOX_MIN = 2500000000000000; // 25 bps
    uint256 constant ARB_TR_MIN = 4000000000000000; // 40 bps of reserveY
    uint256 constant ARB_RET_MIN = 3333333333333333; // 33 bps vs pHat

    uint256 constant SHIELD_TRIGGER = 3003003003003003; // ~30 bps
    uint256 constant SHIELD_BUFFER = 1000000000000000; // 10 bps

    uint256 constant TAIL_KNEE_RAW = 5000000000000000; // 50 bps (raw tradeRatio knee)
    uint256 constant TAIL_CAP_RAW = 100000000000000000; // 10% of reserveY (keep tail signal)
    uint256 constant TAIL_QUAD_COEF = 250000000000000000; // convex tail fee
    uint256 constant TAIL_CUBIC_COEF = 83333333333333333; // extra convexity
    uint256 constant TAIL_RETAIL_MULT_SAMESTEP = WAD + (WAD / 2); // 1.5x

    // --- fee model constants ---
    uint256 constant BASE_FEE = 300000000000000;
    uint256 constant SIGMA_COEF = 200000000000000000; // 0.20
    uint256 constant LAMBDA_COEF = 1200000000000000;
    uint256 constant FLOW_SIZE_COEF = 484200000000000000;
    uint256 constant TOX_COEF = 25000000000000000;
    uint256 constant TOX_QUAD_COEF = 1170000000000000000;
    uint256 constant ACT_COEF = 9184300000000000000;
    uint256 constant DIR_COEF = 2000000000000000;
    uint256 constant DIR_TOX_COEF = 10000000000000000;
    uint256 constant STALE_DIR_COEF = 685000000000000000;
    uint256 constant SIGMA_TOX_COEF = 50000000000000000;
    uint256 constant TAIL_KNEE = 50000000000000000;
