...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant SKEW_FAIR_W = 5;
    uint256 private constant SKEW_INV_W = 6;
    uint256 private constant SKEW_W_SUM = 11;

    uint256 private constant MID_LAMBDA_BASE = 95e16;
    uint256 private constant MID_LAMBDA_DIV = 8e16;
    uint256 private constant MID_ARB_BASE = 45e16;
    uint256 private constant MID_ARB_DIV = 6e16;
    uint256 private constant MID_VOL_BASE_BPS = 9;
    uint256 private constant MID_VOL_DIV = 2;
    uint256 private constant MID_STEP_PENALTY_BPS = 2;
    uint256 private constant MID_ARB_BONUS_BPS = 2;
    uint256 private constant MID_FLOOR_BPS = 8;

    uint256 private constant FAIR_T1_BPS = 8;
    uint256 private constant FAIR_T2_BPS = 16;
    uint256 private constant FAIR_T3_BPS = 30;
    uint256 private constant FAIR_S1_BPS = 2;
    uint256 private constant FAIR_S2_BPS = 4;
    uint256 private constant FAIR_S3_BPS = 7;

    uint256 private constant INV_T1_BPS = 180;
    uint256 private constant INV_T2_BPS = 360;
    uint256 private constant INV_T3_BPS = 620;
    uint256 private constant INV_T4_BPS = 900;
    uint256 private constant INV_S1_BPS = 3;
    uint256 private constant INV_S2_BPS = 5;
    uint256 private constant INV_S3_BPS = 7;
    uint256 private constant INV_S4_BPS = 9;

    uint256 private constant ARB_ANCHOR_GATE_BPS = 240;
    uint256 private constant ARB_MOVE_FLOOR_BPS = 8;
    uint256 private constant SIZE_RELAX_GATE_MULT = 140;
    uint256 private constant SIZE_RELAX_MOVE_MULT = 65;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        int256 skewTarget = _clampSigned(
            (fairSign * int256(SKEW_FAIR_W) + invSign * int256(SKEW_INV_W)) / int256(SKEW_W_SUM),
            -int256(MAX_SKEW_BPS),
            int256(MAX_SKEW_BPS)
        );
    ...
    function getName() external pure override returns (string memory) {
        return "PowellDirect_NoShare_v2";
    }
    ...
    function _classifyProbableArb(
        ...
        if (smallAbs || smallRel) {
            gateMult = SIZE_RELAX_GATE_MULT;
            moveMult = SIZE_RELAX_MOVE_MULT;
        }
    ...
    function _midTargetBps(
        ...
        mid += (int256(lambdaHat) - int256(MID_LAMBDA_BASE)) / int256(MID_LAMBDA_DIV);
        mid += (int256(arbHat) - int256(MID_ARB_BASE)) / int256(MID_ARB_DIV);

        uint256 volBps = wadToBps(volHat);
        mid += (int256(volBps) - int256(MID_VOL_BASE_BPS)) / int256(MID_VOL_DIV);

        if (stepTrades >= 2) mid -= int256(MID_STEP_PENALTY_BPS);
        if (probableArb) mid += int256(MID_ARB_BONUS_BPS);
        return _clampSignedToUint(mid, MID_FLOOR_BPS, TARGET_MAX_BPS);
    }

    function _fairTiltBps(uint256 staleWad) internal pure returns (uint256) {
        if (staleWad > bpsToWad(FAIR_T3_BPS)) return FAIR_S3_BPS;
        if (staleWad > bpsToWad(FAIR_T2_BPS)) return FAIR_S2_BPS;
        if (staleWad > bpsToWad(FAIR_T1_BPS)) return FAIR_S1_BPS;
        return 0;
    }

    function _invTiltBps(uint256 invAbsWad) internal pure returns (uint256) {
        uint256 b = wadToBps(invAbsWad);
        if (b > INV_T4_BPS) return INV_S4_BPS;
        if (b > INV_T3_BPS) return INV_S3_BPS;
        if (b > INV_T2_BPS) return INV_S2_BPS;
        if (b > INV_T1_BPS) return INV_S1_BPS;
        return 0;
    }
