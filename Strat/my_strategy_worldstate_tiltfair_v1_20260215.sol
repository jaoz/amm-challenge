...
/// Fair-aware world-state strategy:
/// - infers likely arb / retail states from first-touch transitions
/// - estimates fair proxy pHat
/// - applies directional skew from spot-vs-fair mispricing
/// - overlays inventory skew, reduced when it conflicts with fair skew
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant A_PRICE_ARB = 16e16; // 0.16
    uint256 private constant A_PRICE_SPOT = 5e16; // 0.05

    // Fair-mispricing tilt (bps thresholds and bps tilt sizes)
    uint256 private constant FAIR_T1 = 8;
    uint256 private constant FAIR_T2 = 16;
    uint256 private constant FAIR_T3 = 28;
    uint256 private constant FAIR_S1 = 2;
    uint256 private constant FAIR_S2 = 4;
    uint256 private constant FAIR_S3 = 7;

    // Inventory tilt (imbalance thresholds in bps, tilt sizes in bps)
    uint256 private constant INV_T1 = 220;
    uint256 private constant INV_T2 = 450;
    uint256 private constant INV_T3 = 700;
    uint256 private constant INV_S1 = 2;
    uint256 private constant INV_S2 = 4;
    uint256 private constant INV_S3 = 7;
    uint256 private constant INV_CONFLICT_DIV = 2;

    // small step-level nudges
    uint256 private constant FIRST_RETAIL_BUMP = 1;
    uint256 private constant ARB_RELAX = 2;
    ...
    function getName() external pure override returns (string memory) {
        return "WorldState_TiltFair_v1_20260215";
    }
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (bidPrev == 0) bidPrev = bpsToWad(40);
        if (askPrev == 0) askPrev = bpsToWad(40);
        ...
        // Arb is first-touch only for a given step.
        if (stepTrades == 1) {
            ...
            if (directionOk && relToHat <= bpsToWad(130)) {
                probableArb = true;
            }
        }
        ...
        // Fair-mispricing tilt.
        uint256 stale = _relDiff(spotPost, pHat);
        bool fairPreferSellX = spotPost < pHat; // underpriced X -> encourage AMM sells X
        ...
        // Inventory tilt; reduce if conflicting with fair-direction correction.
        ...
        // First touch looked retail: small spread bump.
        ...
        bidBps = _clampBps(bidBps, 12, 100);
        askBps = _clampBps(askBps, 12, 100);
    ...
    function _baseFromState(
        ...
        uint256 base = tradeCount < 180 ? 36 : 40;

        if (volHat > bpsToWad(104) / 10) {
            if (base > 10) base -= 10;
        } else if (volHat > bpsToWad(96) / 10) {
            if (base > 5) base -= 5;
        } else if (volHat < bpsToWad(86) / 10) {
            base += 16;
        } else if (volHat < bpsToWad(92) / 10) {
            base += 7;
        }
