...
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant DIR_DECAY = 617422389777183570; // 0.80
    uint256 constant ACT_DECAY = 700000000000000000; // 0.70
    uint256 constant SIZE_DECAY = 700000000000000000; // 0.70
    uint256 constant TOX_DECAY = 861872007207284084; // 0.91
    uint256 constant SIGMA_DECAY = 767455897236310988; // 0.824
    uint256 constant LAMBDA_DECAY = 992215351636490007; // 0.99
    uint256 constant SIZE_BLEND_DECAY = 857664687756180545; // 0.818
    uint256 constant SIZE_SMALL_DECAY = 1000000000000000000; // 0.98  [FIX 2] sub-threshold blend
    uint256 constant TOX_BLEND_DECAY = 254831276376977622; // 0.78  [FIX 3] proper EMA (was 0.051)
    uint256 constant ACT_BLEND_DECAY = 985000000000000000; // 0.985
    uint256 constant PHAT_ALPHA = 209752689373923265; // 0.26
    uint256 constant PHAT_ALPHA_RETAIL = 51629667427686564; // 0.05
    uint256 constant DIR_IMPACT_MULT = 2;

    // --- [FIX 6] Gap-aware pHat update ---
    uint256 constant GAP_PHAT_ALPHA_BOOST = 19329452907430691; // 0.03 per elapsed step beyond 1
    uint256 constant GAP_GATE_PER_STEP = 240959216991731597; // 0.25 WAD widening per elapsed step

    // --- [FIX 1] Opening Quote: 30 bps - 5 units ---
    uint256 constant OPENING_QUOTE = 2999999999999995; // 30 bps = 3e15; minus 5 LSB

    // --- Adaptive Shock Gate ---
    uint256 constant GATE_SIGMA_MULT = 8160804639983958881;
    uint256 constant MIN_GATE = 30621741532974548; // 0.03 WAD

    // --- Cubic Toxicity ---
    uint256 constant TOX_CUBIC_COEF = 731022486607113541;

    // --- Trade-Tox Boost ---
    uint256 constant TRADE_TOX_BOOST = 252648479624540606;

    // --- Asymmetric Stale Dir [FIX 7: added to mutable list] ---
    uint256 constant STALE_ATTRACT_FRAC = 1079715836418863933; // 1.124

    // --- state caps ---
    uint256 constant RET_CAP = 185970026405618224; // 10%
    ...
    uint256 constant ARB_TOX_MIN = 2501422455879020; // 25 bps
    ...
    uint256 constant ARB_RET_MIN = 2940378182133611; // 33 bps vs pHat

    uint256 constant SHIELD_TRIGGER = 3594457170174176; // ~30 bps
    uint256 constant SHIELD_BUFFER = 227222080226152; // 10 bps
    ...
    // --- fee model constants ---
    uint256 constant BASE_FEE = 502522667568376;
    uint256 constant SIGMA_COEF = 107922815653323796; // 0.111
    uint256 constant LAMBDA_COEF = 1223706197482936;
    uint256 constant FLOW_SIZE_COEF = 368498424236280677;
    uint256 constant TOX_COEF = 20289444504250786;
    uint256 constant TOX_QUAD_COEF = 3116265841359117934;
    uint256 constant ACT_COEF = 10851200000000000000;
    uint256 constant DIR_COEF = 1973333925646282;
    uint256 constant DIR_TOX_COEF = 14065046592076984;
    uint256 constant STALE_DIR_COEF = 654836238235190075; // [FIX 7] in mutable list
    uint256 constant SIGMA_TOX_COEF = 40067676950230124;
    uint256 constant TAIL_KNEE = 15200000000000000;
    uint256 constant TAIL_SLOPE_PROTECT = 885296061976545799; // 0.93
    uint256 constant TAIL_SLOPE_ATTRACT = 885296061976545799; // 0.955
    ...
    function getName() external pure override returns (string memory) {
        return "Theo1LocalBest_D11_GAP_PHAT_ALPHA_BOOST_DN24_TOX_BLEND_DECAY_DN14_SIZE_BLEND_DECAY_DN12_1922477E";
    }
}
