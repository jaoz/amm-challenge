...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Safety cap for concurrently launched optimizer workers.",
    )
    parser.add_argument(
        "--spawn-stagger-seconds",
        type=float,
        default=8.0,
        help="Delay between worker launches to avoid memory spikes.",
    )
    parser.add_argument(
        "--sim-workers",
        type=int,
        default=0,
        help="Per-evaluation simulator workers used inside each optimizer worker (0 = auto cap).",
    )
    parser.add_argument(
        "--total-workers-hint",
        type=int,
        default=1,
        help="Total optimizer workers expected to run concurrently (used for auto sim-worker cap).",
    )
...
def append_run_log(out_dir: Path, message: str) -> None:
...
def worker_main(args: argparse.Namespace) -> int:
    ...
    host_workers = max(1, resolve_n_workers())
    total_workers_hint = max(1, int(args.total_workers_hint))
    # Avoid nested oversubscription: cap simulator concurrency per worker so
    # aggregate evaluator workers stay close to host capacity.
    if args.sim_workers > 0:
        sim_workers = max(1, min(int(args.sim_workers), host_workers))
    else:
        sim_workers = max(1, host_workers // total_workers_hint)
    ...
    print(
        ...
        f"global_parent_prob={args.global_parent_prob:.2f} sim_workers={sim_workers} host_workers={host_workers}",
        flush=True,
    )
    write_json_line(
        progress_path,
        {
            ...
            "total_workers_hint": total_workers_hint,
        },
    )

    # Seed incumbent from exact base strategy source.
    print(f"[worker {wid}] base_eval start refine_sims={args.refine_sims}", flush=True)
    base_t0 = time.time()
    base_metrics = evaluate(base_source, args.refine_sims, compiler, baseline, sim_workers, train_eval_seeds)
    base_holdout_metrics = (
        evaluate(base_source, args.refine_sims, compiler, baseline, sim_workers, holdout_eval_seeds)
        if holdout_eval_seeds
        else None
    )
    print(f"[worker {wid}] base_eval done elapsed_s={time.time() - base_t0:.1f}", flush=True)
    ...
    while time.time() < deadline and (iter_budget <= 0 or quick_done < iter_budget):
        ...
        q_metrics = evaluate(source, args.quick_sims, compiler, baseline, sim_workers, train_eval_seeds)
        ...
        r_metrics = evaluate(r_source, args.refine_sims, compiler, baseline, sim_workers, train_eval_seeds)
        ...
        r_holdout_metrics = (
            evaluate(r_source, args.refine_sims, compiler, baseline, sim_workers, holdout_eval_seeds)
            if holdout_eval_seeds
            else None
        )
...
def launch_main(args: argparse.Namespace) -> int:
    ...
    append_run_log(
        out_dir,
        f"launch_start base={args.base_strategy} workers={args.workers} sim_workers={args.sim_workers} quick={args.quick_sims} refine={args.refine_sims}",
    )
    train_eval_seeds, holdout_eval_seeds = resolve_seed_sets(args)
    shared_eval_seed = train_eval_seeds[0]

    safe_cap = max(1, int(args.max_safe_workers))
    requested_workers = int(args.workers)
    workers = max(1, min(requested_workers, safe_cap))
    if workers != requested_workers:
        append_run_log(out_dir, f"workers_clamped requested={requested_workers} effective={workers} safe_cap={safe_cap}")

    worker_iterations = _distribute_iterations(args.iterations, workers)
    worker_python = Path(sys.executable)
    if sys.platform.startswith("win"):
        pyw = worker_python.with_name("pythonw.exe")
        if pyw.exists():
            worker_python = pyw
    script = Path(__file__).resolve()
    procs: list[dict[str, Any]] = []

    def _spawn_worker_detached(cmd: list[str], out_f: Any, err_f: Any) -> subprocess.Popen:
        # Spawn workers independently from the launcher/IDE process tree.
        # This prevents IDE OOM or terminal lifecycle from taking workers down.
        if sys.platform.startswith("win"):
            creationflags = (
    ...
    for wid in range(workers):
        ...
        proc = _spawn_worker_detached(cmd, out_f, err_f)
        append_run_log(
            out_dir,
            f"worker_spawned worker_id={wid} pid={proc.pid} stdout={stdout_path} stderr={stderr_path}",
        )
        if args.spawn_stagger_seconds > 0 and wid < workers - 1:
            time.sleep(args.spawn_stagger_seconds)
        procs.append(
            {
                "worker_id": wid,
                "pid": proc.pid,
    ...
    append_run_log(out_dir, f"launch_complete manifest={manifest_path}")
    print(
        f"Launch complete: workers={workers} sim_workers={args.sim_workers} "
        f"quick={args.quick_sims} refine={args.refine_sims} out={out_dir}"
    )
...
def collect_main(args: argparse.Namespace) -> int:
    out_dir = Path(args.out_dir)
    append_run_log(out_dir, "collect_start")
    ...
    for path in sorted(out_dir.glob("**/best.json")):
        try:
            ...
            continue
    if not bests:
        append_run_log(out_dir, "collect_no_worker_best_files")
    ...
    append_run_log(
        out_dir,
        f"collect_complete best_worker={best_path.parent.name} edge={best_data.get('refine_metrics', {}).get('avg_edge')}",
    )
    return 0
